...
$loop = new UVLoop();
$pipe = new UVPipe($loop);
$pipe->listen($socketName, function($pipe2, $status) use($pipe){
...
$result = $pipe->listen($socketName, function($pipe2, $status) use($pipe){
    echo "on connect $status\n";
    var_dump($pipe === $pipe2);
    $client = $pipe->accept();
    $client->setCallback(function($clientTest, $data) use($client){
