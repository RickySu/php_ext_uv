...
Equal(0, $server->listen($host, $port, function($server) use($server){

    $client = $server->accept();
    $client->setSSLHandshakeCallback(function($clientTest, $status) use($server, $client){
    ...
    $client->setCallback(function($clientTest, $recv) use($server, $client){
        $client->write($recv);    
        if($recv == '!close!'){
            $server->close();
        }
        ...
        $client->close();
    }, function($clientTest) use($server, $client){
